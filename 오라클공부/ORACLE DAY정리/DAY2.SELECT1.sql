...
-- 직원(EMOLOYEE) 테이블에 저장된 전체 데이터 조회하려면
...
-- 컬렴명 AS 별칭, 계산식 AS 별칭
-- AS는 생략할 수 있음. ==> 컬럼명 별칭, 계산식 별칭
-- 주의사항: 별칭에 숫자, 공백, 기호가 사용될 때는 반드시 "별칭" (""로 묵어야 함)
-- 별칭의 글자는 30바이트이하로 작성할 수 있음.
SELECT EMP_ID AS 사번, EMP_NAME 이름, SALARY 급여,
            SALARY * 12 "1년 급여",
            (SALARY + (SALARY * NVL(BONUS_PCT, 0))) * 12 "보너스포인트적용 연봉(원)"
FROM EMPLOYEE;

SELECT LENGTHB('한글') 한글, LENGTHB('ORACLE') 영어
FROM DUAL;  -- DUAL은 가짜 테이블. 테이블이 존재 하지 않을때 사용하는 것
                    -- 오라클 정품에서는 한글 1글자가 2바이트임. XE 에서는 한글 1글자가 3바이트임.

-- SELECT 절에 리터럴(LITERAL, 임의의 문자열값) 사용 할 수 있음. 
-- 가상의 컬럼이 하나 추가됨
-- SELECT 한 데이터들의 상태값 표사용으로 이용할 수 있음
-- '리터럴문자열' AS 별칭
