...
-- SELECT 절에서 조회 컬럼값들의 연결 처리를 해서 하나의 문장을 만들거나
...
SELECT EMP_ID 이름, HIRE_DATE || '입사' 입사일, JOB_ID 직급코드, DEPT_ID 부서코드, 
            SALARY || '(원)' 급여, '기혼' 결혼여부
FROM EMPLOYEE
WHERE HIRE_DATE >= '00/01/01' AND MARRIAGE = 'Y';

-- BETWEEN AND 연산자
-- WHERE 컬렴명 BETWEEN 작은값 AND 큰값
-- 컬럼의 값이 작은값 이상이면서 큰값이하인 행들을 골라라. 라는 의미임.
-- WHERE 컬렴명 >= 작은값 AND 컬럼명 <= 큰값   과 같은 의미의 연산자임.
...
-- 컬럼명 = NULL : 에러임 -> NULL은 값이 아니라 상태이므로 IS NULL을 사용해야함
...
FROM EMPLOYEE
WHERE DEPT_ID IS NULL AND MGR_ID IS NOT NULL;
...
FROM EMPLOYEE
WHERE (DEPT_ID = '90' OR DEPT_ID = '60')
            AND SALARY > 3000000;

-- 해결 : IN 연산자 사용
SELECT EMP_ID, DEPT_ID, SALARY
FROM EMPLOYEE
WHERE DEPT_ID IN ('90', '60')
            AND SALARY > 3000000;
...
-- 문자형(CHAR, VARCHAR2, LONG, CLOB), 숫자형(NUMBER), 날짜형(DATE), : 모든 자료형에 사용 할 수 있음
...
-- 전 직원의 급여 평균 보다 급여를 많이 받는 직원 정보 조회
...
SELECT UPPER(EMAIL), SUM(SALARY)    -- ERROR  이거는 사각형으로 결과가 안나옴!!
FROM EMPLOYEE;
...
-- 이메일에서 '.' 문자 바로 뒤에 있는 소문자 'c' 문자의 위치를 조회
