...
-- 명령여 : CREATE(생성), ALTER(변경), DROP(제거)
...
-- 이름으로 관리된 : CONSTRAINT 저장할이름 제약조건종류 -- 컬럼레벨
-- 이름으로 관리된 : CONSTRAINT 저장할이름 제약조건종류 (적용할 컬럼명) -- 테이블레벨
-- 이름 지정이 생략되면 자동으로 이름이 지정됨 : SYS_C************ 의 형식임
-- 테이블레벨에서, 여러 개의 컬럼을 묶어서 하나의 제약조건을 지정할 수도 있음 (복합키 라고 함)
--        이름으로 관리된 : CONSTRAINT 저장할이름 제약조건종류(컬럼명, 컬럼명, 컬렴명, ........) -- 테이블레벨
...
INSERT INTO TESTUN VALUES ('AAA', '자바');    -- 기록되어 있는 값을 중복 기록 : 에러 
...
CREATE TABLE TESTUN3 (
    ...
    UN_CODE CHAR(2),
    CONSTRAINT UN_TUN3_COMP UNIQUE (U N_ID, UN_NAME)   -- 복합키
);
...
-- 제약조건 : PRIMARY KEY --------------------------------------------
-- 기본키 라고함. (데이터베이스 정규화 과정에서는 IDENTIFIER (식별자)를 말함)
...
INSERT INTO TESTPK VALUES (2, '이순신', SYSDATE);  -- ERROR : UNIQUE
...
INSERT INTO TESTCHK VALUES ('LG G7', 125000, 'B');  -- ERROR : C_PRICE 값 범위 초과
...
CREATE TABLE TESTCHK2(
    ...
    --C_DATE  DATE    CHECK (C_DATE < SYSDATE)    -- ERROR : 제한값이 바뀌는함수는 사용 못함
...
-- REFERENCES 할 컬럼은 반드시 PRIMARY KEY | UNIQUE 제약조건이 설정되어야 함.
...
CREATE TABLE TESTFK2(
    FID CHAR(3)     REFERENCES TEST_NOPK,   -- 참조컬럼 생략되면 자동으로 PK컬럼이 참조됨 : ERROR
    FNAME VARCHAR2(15)
);
...
CREATE TABLE TEST_NFK3(
    FID CHAR(3)     REFERENCES TEST_NOPK2,   
    FNAME VARCHAR2(15) UNIQUE
);

CREATE TABLE TESTFK3(
    FID CHAR(3)     REFERENCES TEST_NOPK2,  -- PK 컬럼 자동 참조
    FNAME VARCHAR2(15) REFERENCE TEST_NOPK3 (NAME)
);

CREATE TABLE CONSTRAINT_EMP(
    EID     CHAR(3) CONSTRAINT PKEID PRIMARY KEY,
    ENAME   VARCHAR2(20) CONSTRAINT NENAME NOT NULL,
    ENO     CHAR(14) CONSTRAINT NOT NULL CONSTRAINT UENO UNIQUE,
    EMAIL   CHAR(14) CONSTRAINT NENO NOT NULL CONSTRAINT UENO UNIQUE,
    PHONE   VARCHAR(12),
    HIRE_DATE   VARCHAR(12),
    HIRE_DATE DATE DEFAULT SYSDATE,
    JID     CHAR(2) CONSTRAINT FKJID    REFERENCE JOB ON DELETE SET NULL,
    SALARY  NUMBER,
    BONUS_PCT   NUMBER,
    MARRAIGE    CHAR(1) DEFAULT 'N' CONSTRAINT CHK CHECK (MARRAIGE IN ('Y', 'N')),
...
-- 즉, EMPLOYEE.DEPT_ID에 기록된 '90' 부서코드가 있으면, DEPARTMENT.DEPT_ID 의 '90' 부서는 삭제 못한다는 의미임.

- DML : DELETE 문
-- 행 삭제 구문임 (행 갯수가 줄어듦)
-- 사용형식 : DELETE FROM 테이블명 WHERE 컬럼명 비교연산자 삭제할 조건값;
-- 조건에 해당되는 값이 있는 행을 삭제하라는 의미임.

DELETE FROM DEPARTMENT
WHRER DEPT_ID = '90';   -- 삭제 불가능, ERROR
...
INSERT INTO PRODUCT VALUES ('갤럭시21', 12000000, 'B');
...
INSERT INTO PRODUCT2 VALUES ('갤럭시21', 12000000, 'B');
...
SELECT * FROM PRODUCT2_STATE;  -- 부모키 삭제 확인 
...
CREATE TABLE TESTFK4(
    ...
    FOREIGN KEY (FID, FNAME) REFERENCE TEST_COMP -- 복합키는 복합키로 설정해야 함
);
