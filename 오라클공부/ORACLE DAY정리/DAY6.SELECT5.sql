...
-- 정룔조건 : ASC(오름차순, 기본값이므로 생략할 수 있음), DESC(내림차순)
...
-- 2000년 1월 1일 이후 입사한 직원 조회
-- 이름, 입사일, 부서코드, 급여 : 별칭 처리
-- 부서코드 기준 내림차순정렬하고, 같은 부서코드일때는 입사일 기준 오름차순정렬, 입사일이같으면 이름 기준 오름차순정렬 처리함
...
-- 직급코드에 대해 오름차순정렬 (NULL 은 아래쪽에 배치함)
...
-- GROUB BY 절에는 그룹핑을 위한 계산식을 사용할 수도 있음
...
-- 사용형식: HAVING 그룹함수(그룹핑한 컬렴명) 비교연산자 비교값
...
SELECT DEPT_ID 부서, SUM(SALARY) "부서별 급여합계"
FROM EMPLOYEE
GROUP BY DEPT_ID
...
-- ROLLUP 사용함
-- NULL 칸은 제외함
SELECT DEPT_ID 부서, JOB_ID 직급코드, SUM(SALARY) "부서별 급여합계"
FROM EMPLOYEE
...
-- ************************************************************************************************
-- 조인 (JOIN)
-- 여러 개의 테이블을 합쳐서 큰 테이블을 만든 다음 필요한 컬럼들을 조회하는 방식임.
...
-- 조인은 연결되는 컬럼의 값이 일치하는 행들만 조인이 됨.
...
-- NON EQUAL JOIN
-- 지정하는 컬럼의 값이 일치하는 경우가 아닌, 값의 범위에 해당되는 행들을 연결하는 방식의 조언

-- 직원들의 급여에 대한 급여등글 매기고자 할 경우
...
-- 첫번째와 두번째가 조인되고 나서, 조긴결과에 세번째가 조인되는 구조임.
...
SELECT EMP_ID 사번, EMP_NAME 이름, JOB_TITLE 직급명, 
           DEPT_NAME 부서명, LOC_DESCRIBE 근무지역명, SALARY 급여
...
JOIN DEPARTMENT USING (DEPT_ID)
JOIN JOB USING (JOB_ID)
WHERE EMP_NO LIKE '6%' AND DECODE( SUBSTR(EMP_NO, 8, 1), '2', '여자', '4', '여자', '남자') = '여자'; 
...
AND EMP_NO LIKE '6%' AND DECODE( SUBSTR(EMP_NO, 8, 1), '2', '여자', '4', '여자', '남자') = '여자'; 

-- 3. 가장 나이가 적은 직원의 
-- 사번, 사원명, 나이, 부서명, 직급명을 조회하시오.
-- 오라클 전용구문
SELECT EMP_ID 사번, EMP_NAME 이름, 
           SUBSTR(EMP_NO, 1, 2) || '년생' 나이,
           DEPT_NAME 부서명, JOB_TITLE 직급명
FROM EMPLOYEE E, DEPARTMENT D, JOB J
WHERE E.DEPT_ID = D.DEPT_ID
AND E.JOB_ID = J.JOB_ID;
...
--나이의 최소값 조회
SELECT MAX(SUBSTR(EMP_NO, 1, 2)) || '년생' 나이
FROM EMPLOYEE;
...
-- 사번, 사원명, 부서명을 조회하시오.
-- 오라클 전용구문
SELECT EMP_ID 사원, EMP_NAME 사원명, DEPT_NAME 부서명
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_ID = D.DEPT_ID
AND EMP_NAME LIKE '성%';
...
-- 오라클 전용구문
SELECT EMP_NAME 사원명, JOB_TITLE 직급명, D.DEPT_ID 부서코드, DEPT_NAME 부서명
FROM EMPLOYEE E, DEPARTMENT D, JOB J
WHERE E.DEPT_ID = D.DEPT_ID
AND E.JOB_ID = J.JOB_ID
...
SELECT EMP_NAME 사원명, BONUS_PCT 보너스포인트, 
           DEPT_NAME 부서명, LOC_DESCRIBE  근무지역명
...
-- self join 사용
-- 오라클 전용구문
SELECT E.EMP_NAME 사원명, E.EMP_ID 부서코드,
           D.EMP_NAME 동료이름, D.EMP_ID 부서코드 
...
SELECT MARRIAGE 혼인여부, COUNT(*) 직원수
FROM EMPLOYEE
