...
-- 다른 직원들의 입사일 데이터 확인SELECT EMP_ID 사번, EMP_NAME 이름,
SELECT TO_CHAR(HIRE_DATE, 'YYYY-MM-DD AM HH24:MI:SS') 입사일
...
-- 글자 각각이 날짜 무엇으로 바뀌는지를 지정하는 역할을 함
-- TO_DATE() 의 포멧은 글자 각각이 날짜 무엇으로 바뀌는지를 지정하는 역할을 함
SELECT TO_DATE('20120101', 'YYYYMMDD')
FROM DUAL; -- DATE 형으로 변환되면 '10/01/01' 로 표현됨

SELECT TO_CHAR( '20100101', 'YYYY, MON')
FROM DUAL; -- 에러 : '20120101' 은 DATE 형이 아님. TO_CHAR()로 포멧 적용할 수 없음
...
FROM DUAL; -- 시간에 대한 포멧율 12시간제이면 HH, 24시간제이면 HH24로 지정함, 공백도 맞추어야 함

SELECT TO_CHAR( TO_DATE( '041030 133000', 'YYMMDD HH24MISS' ), 'DD-MON-YY HH:MI:SS PM' )
FROM DUAL; -- 글자의 공백자리도 맞추어야 함

SELECT TO_DATE( '980630', 'YYMMDD' )
FROM DUAL;

SELECT TO_CHAR( TO_DATE( '980603', 'YYMMDD' ) , 'YYYY.MM.DD')
FROM DUAL;

-- 날짜로 변환시 주의사항
SELECT TO_DATE('20161230', 'YYYYMMDD')
FROM DUAL; -- 35일은 없는 값 이므로...

-- 2022년 10월 9일의 요일은?
SELECT TO_CHAR( TO_DATE( '20221009', 'YYYY-MM-DD'), 'DY' )
FROM DUAL;
...
-- 현재 년도와 바꿀년도가 둘 다 50미만이면, Y | R 아무거나 사용해도 됨
...
-- 프로그래밍의 switch문과 같은 동작 방식임.

-- 50번 부서에 소속된 직원들의 이름과 성별을 조회
-- 성별 기준 : 주민번호 8번째 값이 1, 3 이면 남자, 2, 4 이면 여자로 출력 처리
SELECT EMP_NAME 이름,
...
-- 2. CASE 표현식 사용한 경우
-- CASE 표현식은 다중if문과 같은 동작 구조를 가짐
SELECT EMP_NAME, JOB_ID, SALARY,
            CASE JOB_ID
                WHEN 'J7' THEN SALARY * 1.1
                WHEN 'J6' THEN SALARY * 1.15
                WHEN 'J5' THEN SALARY * 1.6
                ELSE SALARY * 1.05
        END 인상급여
FROM EMPLOYEE;
...
SELECT EMP_NAME 이름, JOB_ID 직급코드,
            TO_CHAR(SALARY*NVL(BONUS_PCT, 1)*12, 'L99999999') "보너스포인트가 적용된 1년치 급여"
FROM EMPLOYEE;

--3. 부서코드가 50, 90인 직원들 중에서 2004년도에 입사한 직원의 
--   수 조회함.
--	사번 사원명 부서코드 입사일
SELECT EMP_ID 사번, EMP_NAME 이름, DEPT_ID 부서코드, HIRE_DATE 입사일
FROM EMPLOYEE
WHERE DEPT_ID IN(50, 90) AND EXTRACT(YEAR FROM HIRE_DATE) = '2004';
