...
-- 문자열의 왼쪽에 있는 문자를 제거함. (제거할 문자가 아닌 다른 문자가 있으면 LTRIM 종료됨)
-- RTRIM('문자열리터럴' | 문자가 기록된 컬럼명, '제거할문자들')
-- 문자열의 오른쪽에 있는 문자를 제거함. (제거할 문자가 아닌 다른 문자가 있으면 LTRIM 종료됨)
SELECT '       0123456ABCDExxxxxyxz'         ,
       LTRIM('       0123456ABCDExxxxxyxz         '),
       LTRIM('       0123456ABCDExxxxxyxz         ', ' '),
       LTRIM('       0123456ABCDExxxxxyxz         ', ' 0123456789'),
       LTRIM('       0123456ABCDExxxxxyxz         ', ' 0123456ABCDExxxxxyxz')
FROM DUAL;

SELECT '       0123456ABCDExxxxxyxz         ',
       RTRIM('       0123456ABCDExxxxxyxz         '),
       RTRIM('       0123456ABCDExxxxxyxz         ', ' '),
       RTRIM('       0123456ABCDExxxxxyxz         ', ' 0123456789'),
       RTRIM('       0123456ABCDExxxxxyxz         ', ' 0123456ABCDExxxxxyxz')
FROM DUAL;
...
-- 별칭 적용
SELECT EMP_NO 주민번호, SUBSTR(EMP_NO, 1, 2) 생년,
...
-- INITCAP('문자리터럴' | 문자가 기록된 컬럼명) : 영문자일 때 첫글자만 대문자로 바꾸는 함수
...
-- 직원 테이블에서 이름, 주민번호 조회
-- 주민번호는 생년월일만 보이게 하고 뒷자리는 '*'로 처리함 : 781225-*******
SELECT EMP_NAME 이름, EMP_NO 주민번호,
       RPAD(SUBSTR(EMP_NO, 1, 8), 14, '*') 주민번호
FROM EMPLOYEE;

-- 숫자 처리 함수 =====================================================
-- ROUND(), TRUNG(), FLOOR(), ABS(), MOD()
...
-- 자릿수 : 생략되면 0임, (0은 소숫점 위치임)
-- 지정한 자릿수까지(양수) 또는 지정한 자릿수(음수)의 값을 버리는 함수임. (반올림 없음)
SELECT 145.678,
       TRUNC(145.678), -- 145
       TRUNC(145.678, 0), -- 145
...
-- 데이터 딕셔너리는 DB시스템이 관리함, 사용자는 손댈 수 없음
-- 사용자는 저장된 정보를 조회해서 볼 수 는 있음
...
-- 지정된 날짜 뒤쪽 날짜에서 가장 가까운 지정 요일의 날짜를 변환함
...
-- LAST_DAY('날짜리터럴' | 날짜가 긹된 컬럼명)
-- 지정한 날짜의 월에 대한 마지막 날짜(DATE)를 번환함
...
-- 직원의 이름, 입사일, 근무년수 조회
-- 근무년수는 오늘 날짜 기준으로 계산함 : EXTRACT, MONTHS_BETWEEN
SELECT EMP_NAME 이름, HIRE_DATE 입사일,
...
-- 자동형변환의 경우
SELECT 25 + '10'    -- NUMBER | CHARACTER -> 자동으로 NUMBER로 바뀜(자동형변환)
FROM DUAL;
...
-- 년도 출력 포멧
SELECT SYSDATE,
            TO_CHAR(SYSDATE, 'YYYY-MM-DD'), TO_CHAR(SYSDATE, 'YYYY'),
...
SELECT SYSDATE,
            TO_CHAR(SYSDATE, 'YYYY "년"'), TO_CHAR(SYSDATE, 'YYYY'),
            TO_CHAR(SYSDATE, 'YY'), TO_CHAR(SYSDATE, 'RR'),
            TO_CHAR(SYSDATE, 'YEAR')
FROM DUAL;
...
-- 월에 대한 포멧
SELECT SYSDATE,
            TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일"'), TO_CHAR(SYSDATE, 'YYYY'),
...
SELECT EMP_NAME 이름,
            TO_CHAR(HIRE_DATE, 'YYYY"년" MM"월" DD"일" "("DY")"') "입사일1",
            TO_CHAR(HIRE_DATE, 'YYYY"년" MON DD"일" "("DY")"') "입사일2"
FROM EMPLOYEE;
